...
public class MySQLStorage implements Storage {
    ...
    public void connect() {
        ...
        try {
            ...
            config.setPoolName("Duels-MySQL");
            ...
            this.asyncExecutor = createExecutor("duels-mysql");
            createTablesAndMigrations();
            plugin.getLogger().info("Connected to MySQL via HikariCP.");
        } catch (Exception e) {
            logSqlError("Could not connect to MySQL", e);
            closeDataSource();
        }
    }
    ...
    private Connection getConnection() throws Exception {
        if (!isConnected()) {
            throw new IllegalStateException("Storage is not connected.");
        }
        return dataSource.getConnection();
    }

    private PreparedStatement prepare(Connection connection, String sql) throws Exception {
        PreparedStatement statement = connection.prepareStatement(sql);
        statement.setQueryTimeout(queryTimeoutSeconds);
        return statement;
    }

    private void createTablesAndMigrations() throws Exception {
        try (Connection connection = getConnection();
                ...
                PreparedStatement punishments = prepare(connection, SqlQueries.CREATE_PUNISHMENTS_MYSQL)) {
    ...
    private void logSqlError(String action, Exception exception) {
        plugin.getLogger().warning("[MySQL] " + action + ": " + exception.getMessage());
    }
    ...
    public CompletableFuture<List<me.raikou.duels.punishment.Punishment>> getActivePunishments(UUID uuid) {
        return CompletableFuture.supplyAsync(() -> {
            ...
            String sql = "SELECT * FROM duels_punishments WHERE uuid=? AND active=true AND removed=false";
            try (Connection connection = getConnection();
                    PreparedStatement ps = prepare(connection, sql)) {
                ps.setString(1, uuid.toString());
                try (ResultSet rs = ps.executeQuery()) {
    ...
    public CompletableFuture<Void> expirePunishment(int id, String removedBy, String reason) {
        return CompletableFuture.runAsync(() -> {
            String sql = "UPDATE duels_punishments SET active=false, removed=true, removed_by=?, removed_reason=? WHERE id=?";
            try (Connection connection = getConnection();
                    PreparedStatement ps = prepare(connection, sql)) {
